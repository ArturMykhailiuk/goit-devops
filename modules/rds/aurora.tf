...
resource "aws_rds_cluster" "aurora" {
  ...
  # Pre-destroy hook для видалення старих snapshot'ів
  provisioner "local-exec" {
    when    = destroy
    command = "aws rds delete-db-cluster-snapshot --db-cluster-snapshot-identifier ${self.cluster_identifier}-final-snapshot 2>/dev/null || true"
  }
